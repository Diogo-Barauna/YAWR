...
chrome.runtime.onMessage.addListener(async function (message) {
    if (message.action === "atualizarRelogio") {
        if (!coposRecomendados <= 0) {
            clockDisplay.innerHTML = "O alarme tocará em " + message.tempo + " minutos"
        } else {
...
let valor = chrome.storage.local.get(['valorRelogio'], function (result) {
    if (!coposRecomendados <= 0) {
        clockDisplay.innerHTML = "O alarme tocará em " + result.valorRelogio + " minutos"
    }
});
...
document.addEventListener('click', e => {
    ...
    if (el.classList.contains('resetar')) {
        if (!coposRecomendados <= 0) {
            chrome.runtime.sendMessage({ action: "resetarTimer" });
        } else {
            alert('Você já atingiu sua quantidade diária recomendada, o alarme não pode ser resetado')
        }
    }
})
